...
extern const struct VMStateDescription vms_nes6502_cpu;

void nes6502_cpu_do_interrupt(CPUState *cpu);
bool nes6502_cpu_exec_interrupt(CPUState *cpu, int int_req);
hwaddr nes6502_cpu_get_phys_page_debug(CPUState *cpu, vaddr addr);
int nes6502_cpu_gdb_read_register(CPUState *cpu, GByteArray *buf, int reg);
int nes6502_cpu_gdb_write_register(CPUState *cpu, uint8_t *buf, int reg);
int nes6502_print_insn(bfd_vma addr, disassemble_info *info);
vaddr nes6502_cpu_gdb_adjust_breakpoint(CPUState *cpu, vaddr addr);
...
#define cpu_mmu_index nes6502_cpu_mmu_index

static inline int nes6502_cpu_mmu_index(CPUNES6502State *env, bool ifetch)
{
