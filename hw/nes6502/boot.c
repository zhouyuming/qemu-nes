...
{
    address_space_write(&address_space_memory, page_number * 0x2000 + 0x1A000, MEMTXATTRS_UNSPECIFIED, source, 0x2000);
}

static void ppu_set_mirroring(byte mirroring)
{
    address_space_write(&address_space_memory, 0x2000 + 8, MEMTXATTRS_UNSPECIFIED, &mirroring, 1);
}
...
static int fce_load_rom(char *rom, NesMcuState *s)
{
    ...
    if (mmc_id == 0 || mmc_id == 3) {
        if (fce_rom_header.prg_block_count == 1){
            ...
            address_space_write(&address_space_memory, FIRST_CODE_OFFSET, MEMTXATTRS_UNSPECIFIED, buf, PRG_BLOGK_SIZE * 2);
        }
    }
    else {
        return -1;
    ...
    for (i = 0; i < fce_rom_header.chr_block_count; i++) {
        ...
        if (i == 0) {
            address_space_write(&address_space_memory, 0x16000, MEMTXATTRS_UNSPECIFIED, buf, 0x2000);
        }
    }
